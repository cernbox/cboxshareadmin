...
def arg_parser(**kwds):
    ...
    group.add_argument('--quiet', '-q', dest='loglevel', action="store_const", const=logging.ERROR,
                       help='do not produce output (other than errors)')
    group.add_argument('--debug', dest='loglevel', action="store_const", const=logging.DEBUG,
                       help='produce very verbose output')
    # group.add_argument('--verbose', '-v', action="store_true", help='produce more output')

    group = parser.add_mutually_exclusive_group()

    group.add_argument('--json', dest='json', action="store_true", default=False,
                       help='Use JSON as data exchange format (this is an "API" call by another program). Print result on stdout in a JSON format.')
