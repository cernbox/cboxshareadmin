...
def compute_acls(fid, eos, db, owner=None):
        ...
        try:
            ...
            try:
                ...
                if x.returncode == 2:
                    ...
                    logger.warning("DANGLING_SHARE id=%d owner=%s sharee=%s target='%s' fid=%s", s.id, s.uid_owner,
                                   s.share_with, s.file_target, fid)
                    continue
    ...
    logger.debug('descendants: %s',
                 [(path, sorted(descendant_nodes[path].share_acl)) for path in sorted(descendant_nodes)])

    for path in sorted(descendant_nodes):
        acls = ancestor_acl[:]

        # add ACLs of all ancestors of current path which are in the descendant_nodes
        acls.extend(sum([sorted(n.share_acl) for p, n in sorted(descendant_nodes.items()) if is_ancestor(n.file, path)],
                        []))  # flatten the list
...
def collapse_into_nodes(shares):
    ...
    for s in shares:
        nodes.setdefault(s.item_source,
                         ShareNode(inode=s.item_source, owner=s.uid_owner, shares=set(), fileid_prefix=s.fileid_prefix))
...
def list_shares(user, role, groups, fid, share_type, flat_list, include_broken, db, eos):
    ...
    if flat_list:
        ...
        retobj = {}
        for s in shares:
            logger.debug("Processing share: %s %s->%s %s %s", s.id, s.uid_owner, s.share_with, s.item_source,
                         str(s.file_target))
            ...
            try:
                ...
                if x.returncode == 2:
                    # eos entry does not exist
                    logger.warning("DANGLING_SHARE id=%d owner=%s sharee=%s target='%s' inode=%s", s.id, s.uid_owner,
                                   s.share_with, s.file_target, s.item_source)
                    share_path = None

            if share_path or include_broken:
                retobj[s.id] = {'uid_owner': s.uid_owner, 'uid_initiator': s.uid_initiator, 'share_id': s.id,
                                'share_with': s.share_with, 'type': s.share_type, 'target_inode': s.item_source,
                                'target_name': s.file_target, 'permissions': s.permissions,
                                'created': datetime.datetime.fromtimestamp(s.stime).isoformat(),
                                'expires': dtisoformat(s.expiration), 'token': s.token, 'target_path': share_path}
        ...
        for target_id in nodes:
            ...
            if target_path or include_broken:
                retobj.append(
                    {'path': target_path, 'inode': target_id, 'size': target_size, 'shared_by': nodes[target_id].owner,
                     'shared_with': []})
                for s in nodes[target_id].shares:
                    acl = share2acl(s)
                    retobj[-1]['shared_with'].append(
                        {'entity': acl.entity, 'name': acl.name, 'permissions': db2crud(s.permissions),
                         'created': datetime.datetime.fromtimestamp(s.stime).isoformat()})
